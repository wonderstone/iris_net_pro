function Table() {
	...
	TClass.init = function(option) {
		...
		Option[id] = {
			...
			headerBgColor: Tool.getValue(option['headerBgColor'], '#F0FFF0'),
			...
			evenBgColor: Tool.getValue(option['evenBgColor'], '#F0FFF0'),
			oddBgColor: Tool.getValue(option['oddBgColor'], '#F0FFF0'),
			rowHeight: Tool.getValue(option['rowHeight'], 34),
			columnWidth: Tool.getValue(option['columnWidth'], [15,15,15])
		};
		Tool.render(id, tag);
	};
